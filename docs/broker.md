...
Use `await broker.generateResponse(messages, tools, config)` to receive one native gateway response without
...
Set `{ maxToolIterations: null }` to disable the tool-round limit.
